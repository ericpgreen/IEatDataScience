...
We'll use functions from the `tidyr` and `dplyr` packages today, both of which are part of the tidyverse. So start by loading the `tidyverse` package.
...
If you completed the Week 4 tutorial, you should already have the data file you need for today. If not, run the following chunk to download the data:
...
Use `dplyr` to get the mean of `c.age.r1` and `p.age.r1`.
...
The grouping variable is `p.knowsHIV.r1`.

## Now summarize by disclosure status

The grouping variable is `p.knowsHIV.r1` (use a pipe).
...
## Add other variables to your `summarize()` step
...
```{r}
  dat %>%
    ...
    summarise(c.age=mean(c.age.r1, na.rm=TRUE),
              p.age=mean(p.age.r1, na.rm=TRUE),
              p.eduCompPri=mean(p.eduCompPri.r1, na.rm=TRUE),
              c.eduCompPri=mean(c.eduCompPri.r1, na.rm=TRUE),
              p.bio=mean(p.bio.r1, na.rm=TRUE),
...
```{r}
  dat %>%
    ...
    summarise(c.age=mean(c.age.r1, na.rm=TRUE),
              ...
              p.female=mean(p.female.r1, na.rm=TRUE)) %>%
    gather(., key="variable", value="value", -p.knowsHIV.r1) %>%
    mutate(p.knowsHIV.r1 = ifelse(p.knowsHIV.r1==1, 
                                  "disclosed", 
                                  "non-disclosed")) %>%
    spread(., key=p.knowsHIV.r1, value=value)
```
